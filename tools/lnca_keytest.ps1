...
$probe = Join-Path $root "Manager\tools\LncaProbe\bin\Release\net8.0\LncaProbe.exe"
if (-not (Test-Path $probe)) {
    $probe = Join-Path $root "Manager\tools\LncaProbe\bin\Debug\net8.0\LncaProbe.exe"
}
if (-not (Test-Path $probe)) {
    Write-Host "未找到 LncaProbe.exe，请先编译：" -ForegroundColor Yellow
    Write-Host "  dotnet build Manager/tools/LncaProbe/LncaProbe.csproj -c Release"
    exit 1
}
...
if ($hit) {
    ...
    Write-Host "  dotnet run --project Manager/tools/LncaProbe -c Release -- --format $Port <新PIN> <SO口令>"
    exit 0
} else {
