...
def disasm(rva, n=150):
    off = r2o(rva)
    return list(md.disasm(data[off:off + n * 16], rva))

# HD_DeleteCert full body: understand how the two strings map to container/cert selection
def show(name, rva, count=110):
    print('=' * 80)
    print(f'{name} (RVA 0x{rva:X})')
    print('=' * 80)
    for insn in disasm(rva, count):
        extra = ''
