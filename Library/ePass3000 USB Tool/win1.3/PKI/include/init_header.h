...
typedef struct _INIT_TOKEN_PARAM_
{
	...
	BYTE SoPinEC;					// max SO Pin error counter, 0x00 to default(15)
	BYTE userPinEC;					// max User Pin error counter
	unsigned long ulPubSize;		// public storage size in bytes, 0x00 to default(4700)
	unsigned long ulPrvSize;		// private storage size in bytes, 0x00 to default(2000)
