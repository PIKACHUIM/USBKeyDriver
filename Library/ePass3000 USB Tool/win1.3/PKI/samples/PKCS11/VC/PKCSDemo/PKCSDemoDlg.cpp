...
//const CK_ULONG MODULUS_BIT_LENGTH = 1024;	// RSA key length
...
CK_BYTE pbMsg[] = "ePassNG bring you into e-Business time.";
...
BOOL CPKCSDemoDlg::OnInitDialog()
{
	...
	ShowMsg("Before any operation,You need connect to Token!"NEWLINE);
...
void CPKCSDemoDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
void CPKCSDemoDlg::OnBtnConnect() 
{
	...
	if(CKR_OK != rv )
	{
		ShowErr(NEWLINE"Can't Acquire information of ePassNG,Error Code is 0x%08X."NEWLINE, rv);
		return;
	}
	if(0 >= ulCount)
	{
	...
	else
	{
		...
		ShowMsg(NEWLINE"Before next test,need to generate RSA Key Pair!"NEWLINE);
...
void CPKCSDemoDlg::OnBtnKeypairgen() 
{
	...
	if("" == g_strUserPIN)
	{
		ShowMsg(NEWLINE"You Must input user pin before generating RSA Key pair!"NEWLINE);
		return;
	}
	...
	if(CKR_OK != rv)
	{
		ShowErr(NEWLINE"Can't Generate RSA Key Pair,Error code 0x%08X."NEWLINE, rv);
		return;
	}
	else
	{
...
void CPKCSDemoDlg::OnBtnEncrypt() 
{
	...
	ShowMsg(NEWLINE"Data :"NEWLINE);
	ShowMsg((char*)pbMsg);
	ShowMsg(NEWLINE"was encrypted success!"NEWLINE"Encryted Data is:"NEWLINE);
