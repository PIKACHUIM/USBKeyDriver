...
CK_RV CPKCSDemo::Connect() 
{
	...
	if(CKR_OK != rv )
	{
		printf("Can't Acquire information of ePassNG,Error Code is 0x%08X", rv);
		return rv;
	}
	if(0 >= ulCount)
	{
	...
	if(CKR_OK != rv )
	{
		printf("Can't Acquire information of epassNG,Error Code is 0x%08X\n", rv);
		return rv;
	}
	if(0 >= ulCount)
	{
	...
	if(CKR_OK != rv )
	{
		printf("\nCan't Acquire information of ePassNG,Error Code is 0x%08X\n", rv);
		delete[] m_pSlotList;
		m_pSlotList = NULL_PTR;
		return rv;
	}
