...
> **分析工具**：objdump（msys64 mingw64）+ 自研 Python 分析链（`GM3000逆向资料/scripts/gm_*.py`）
> + 自研 x86 实机探针（`GM3000逆向资料/probe`）。
...
5. 需求 ① 的修复**已落地并验证**（见 §七「方案 A 实施结果」）：为 2.2.19 目录提供了
   **基于 SKF 的 PKCS#11 垫片**（`GM3000逆向资料/shim/`，32 位，仅导出
...
探针 `GM3000逆向资料/probe`（x86，32 位；全部为只读调用，未做任何 PIN/解锁/重置写操作）：
...
垫片源码与构建脚本：`GM3000逆向资料/shim/`（`shim.c` / `shim.def` / `build.ps1`）。
产物：`GM3000逆向资料/shim/build/gm3000_pkcs11.dll`（32 位，137,216 字节，仅导出
...
> 因此禁止「两种句柄各试一次」的探测方式，必须先实测确认（`GM3000逆向资料/probe skfhandle`）。

**② PIN 编号反了。** 实测（`GM3000逆向资料/probe skfpin`）：
...
| 文件 | 修改 |
|---|---|
| `GM3000逆向资料/shim/shim.c` | `VerifyPIN/ChangePIN/UnblockPIN/ClearSecureState` 首参改用 `g_curApp`；`CKU_SO → pinType 0`、`CKU_USER → pinType 1`；`C_SetPIN` 按会话身份选择编号 |
...
**端到端复验**（`GM3000逆向资料/probe shimabi`，复刻 Admin 的失败调用）：
...
Copy-Item "GM3000逆向资料\shim\build\gm3000_pkcs11.dll" "<SDK>\GM3000_2.2.19\gm3000_pkcs11.dll"
...
powershell -NoProfile -ExecutionPolicy Bypass -File GM3000逆向资料\shim\build.ps1
```
...
### 8.2 实机验证输出（`GM3000逆向资料/probe provider`，走 Manager 真实代码）
...
| 8 | PKCS#11 垫片所需的 `M_*` 完整签名表 | ✅ 已确认槽位 0/1/4/11/17 的签名与语义（见 §七）；其余槽位签名待补 | `GM3000逆向资料/scripts/tm_ext_sites.py` 定位调用点 + 导出层反汇编 |
...
| 工具 | 用途 |
|---|---|
| `GM3000逆向资料/scripts/gm_pe.py` | 纯 Python PE 解析：`info`（节区/数据目录）/`exports`/`imports`/`strings`/`findva`/`xref` |
| `GM3000逆向资料/scripts/gm_dump_all.py` | 批量落盘 GM3000 全部二进制的导出/导入/特征/关键字符串 → `GM3000逆向资料/disasm/` |
| `GM3000逆向资料/scripts/gm_dis.py` / `gm_disraw.py` | 按导出名 / 按显式 RVA 反汇编（产物落盘，规避 PowerShell 编码与转义问题） |
| `GM3000逆向资料/scripts/gm_sig.py` | 依据函数尾部 `ret imm16` **批量推断导出的调用约定与参数个数** |
| `GM3000逆向资料/scripts/gm_dbg.py` | PE 头部与数据目录原始字节调试（用于定位解析器 bug） |
| `GM3000逆向资料/scripts/gm_hexdump.py` | 十六进制 + 字符串 + 小端整数三视图转储（用于还原回填结构体） |
| `GM3000逆向资料/scripts/gm_modules.py` | 跨 WOW64 枚举指定进程的**32 位模块**（Toolhelp32 + `EnumProcessModulesEx`） |
| `GM3000逆向资料/probe/` | x86 实机探针：`scan` / `p11` / `p11init` / `tmstyle` / `tables` / `skf` / `tm` / `provider` / `all` |
| `GM3000逆向资料/shim/` | **PKCS#11 垫片**（`shim.c` + `shim.def` + `build.ps1`）：基于 SKF 实现 PKCS#11，供厂商 TokenMgr/Admin 使用 |
| `scripts/win_dump_windows.ps1` | 递归枚举指定进程的窗口/控件文本（无法看屏时观察 GUI 状态） |
| `GM3000逆向资料/scripts/tm_find_ext_calls.py` | 检索 TokenMgr 中经扩展表调用 `M_*` 的调用点，用于推断参数约定 |
...
- `Library/Longmai GM3000 SDK/GM3000逆向资料/docs/Longmai-GM3000-逆向分析报告.md`（本文件）
- `GM3000逆向资料/scripts/gm_pe.py`、`gm_dump_all.py`、`gm_dis.py`、`gm_disraw.py`、`gm_sig.py`、`gm_dbg.py`、`gm_hexdump.py`、`gm_modules.py`、`tm_find_ext_calls.py`、`win_dump_windows.ps1`
- `GM3000逆向资料/probe/`（`GM3000Probe.csproj`、`Program.cs`）
- `GM3000逆向资料/shim/`（`shim.c`、`shim.def`、`build.ps1`、`build/gm3000_pkcs11.dll`）
- `GM3000逆向资料/disasm/`（分析产物，可随时重建）
...
| 文件 | 改动 |
|---|---|
| `GM3000逆向资料/shim/shim.c` | ①`C_GetTokenInfo` 前刷新设备快照（名称随设备走）②`C_GetSessionInfo` 如实报 5 种状态（SO=4）③实现槽位 12/13/14/15/16（文件类）④`M_Dispatch`/桩扩到 6 参以承载 5/6 参函数⑤新增 `RestoreSecureState*` 用缓存口令恢复「已验证」状态 |
| `Library/Longmai GM3000 SDK/GM3000_2.2.19/APPInfo.ini` | `CSPLib` / `CSPName` 对齐本机已安装的 HZ 版 CSP（可改回，见 12.4） |
| `GM3000逆向资料/probe/Program.cs` | 新增 `adminfix` / `tmfix` 命令（驱动 Admin 的改名/改密真实入口）、`shimabi` 增加容器+文件槽位回归 |
| `GM3000逆向资料/scripts/gm_exttable.py` | 新增：直接读厂商扩展表静态结构（`M_GetExtFunctionList` 只是把 rva `0x7BB30` 的表指针写回出参），权威还原 25 项槽位顺序 |
...
**实机验证**（`GM3000逆向资料/probe provider`，走 Manager 真实代码）：
...
### 12.7 Admin「导入证书」三步流程与对象记账（0x4 / 权限不足 / 0x30 的最终根因）

厂商 Admin 的「导入证书」在 PKCS#11 层是**三步**（日志逐条实证）：

```
M_CreateContainer('{GUID}')                 → 容器
C_CreateObject(12 属性) class=0x2  CKO_PUBLIC_KEY     ← 公钥对象
C_CreateObject(19 属性) class=0x3  CKO_PRIVATE_KEY    ← 私钥对象（裸 CRT 分量）
C_CreateObject( attr )  class=0x1  CKO_CERTIFICATE    ← 证书对象（真正落卡的一步）
```

垫片此前**只认证书对象**，前两步一律拒绝 → Admin 立刻中止并回滚删容器，
界面把失败翻译成「权限不足，请重新使用口令登录」；即使放行前两步，
Admin 还会**回查**刚建的对象（`C_FindObjectsInit` + `C_GetAttributeValue`），
而旧实现的 `C_FindObjectsInit` **忽略模板**、只回「容器证书」→ 查到 0 个 →
Admin 判创建失败并中止，界面表现为 `ErrorCode=0x4`，**证书那一步永远轮不到**。

修复（对齐 Manager `Gm3000Provider.ImportPfx` 的既有做法）：

| 层 | 处理 |
|---|---|
| `CKO_PUBLIC_KEY` | 记账对象（设备无导入公钥入口），属性可读（CLASS/ID/KEY_TYPE/MODULUS/PUBLIC_EXPONENT/MODULUS_BITS） |
| `CKO_PRIVATE_KEY` | 用模板 CRT 分量现场拼 PKCS#1 DER → **尝试** `SKF_ImportRSAKeyPair`；无论成败都**不阻断**，并登记记账对象 |
| `CKO_CERTIFICATE` | `SKF_OpenContainer` + `SKF_ImportCertificate`（先签名后加密），写前用缓存**用户**口令恢复安全状态 |
| `C_FindObjectsInit` | 按模板（CKA_CLASS / CKA_ID / CKA_LABEL）过滤 |
| `LoadObjects` | 只重建证书对象（`FreeCertObjects`），保留记账对象，避免刚建就被清掉 |

ABI 层回归（`GM3000Probe ckobj`，复刻三步）：

```
① C_CreateObject(CKO_PUBLIC_KEY)  → 0x0 句柄=1；FindObjectsInit(CLASS=PUBLIC_KEY) → 1 个 ✅
② C_CreateObject(CKO_PRIVATE_KEY) → 0x0 句柄=3；FindObjectsInit(CLASS=PRIVATE_KEY) → 1 个 ✅
③ C_CreateObject(CKO_CERTIFICATE) → 未登录+假 DER 时 0x30（设备正常拒绝）
```

### 12.8 名字读取的自身缺陷：提前 return 没补终止符（很隐蔽）

`CopyStr`（读上层传入的容器名/文件名）原本这样写：

```c
c = ((const char*)p)[i];
if (c == 0) return;      /* ← 直接 return */
out[i] = c;
...
out[i] = 0;              /* ← 只有正常走完循环才补 NUL */
```

**提前 return 时 `out[i]` 保持未初始化**，于是传出去的名字后面挂着**栈上残留**，
且每次调用内容不同。后果是同一文件在两次调用里名字不同：

```
M_CreateFile('ZZTEST.DAT…残留A')  → 0x0
M_WriteFile ('ZZTEST.DAT…残留B')  → 0x0A000031  SAR_FILE_NOT_EXIST（名字对不上）
M_DeleteContainer('…残留')        → 0x0A000010  SAR_INDATALENERR（长度非法）
```

> 一度误判为「上层缓冲区不是 NUL 结尾」——实际是**垫片自己读出来的名字被污染**。
> 修复：入口 `memset(out,0,cap)`，并在任何提前退出前写终止符；
> 同时只接受可打印 ASCII（0x20..0x7E），遇到其它字节即结束。

修复后同一轮测试全部自洽（`GM3000Probe shimabi`）：

```
M_CreateContainer('SHIMTEST')      → 0x0A00002D（名字干净；0x2D=需用户角色，设备规则）
M_CreateFile('ZZTEST.DAT')         → 0x0
M_WriteFile ('ZZTEST.DAT', off=0)  → 0x0A00002D（与建文件名字一致）
M_GetFileInfo('ZZTEST.DAT')        → 0x0     ✅
M_DeleteFile('ZZTEST.DAT')         → 0x0     ✅
```

### 12.9 Admin 模块级修复清单（本轮）

| 现象 | 根因 | 修复 | 验证 |
|---|---|---|---|
| 改名称失败 | `C_GetTokenInfo` 的名称取自启动缓存，非设备当前值 | `RefreshDeviceSnapshot()`；名称即 `DEVINFO+0x82`（实测 `SKF_SetLabel` 写的就是这里） | `tmfix` 回读 = 设备当前值 ✅ |
| 改 SO PIN `ErrorCode=0x1` | TokenMgr `token_change_sopin` 要求 `C_GetSessionInfo.state == CKS_RW_SO_FUNCTIONS(4)`，垫片恒回 2/0 | 如实上报 5 种会话状态（SO=4） | `tmfix`：`token_change_sopin` → 0x0 ✅ |
| 删除容器 `0x30` | 名字带残留 → 设备 `SAR_INDATALENERR`，未映射码兜底成 DEVICE_ERROR | 见 12.8 + `C_GetAttributeValue` 的 CKA_ID/CKA_LABEL **带终止 NUL** 返回 | 用户实测删除成功 ✅ |
| 下载 ISO `0x54` | 文件类槽位未实现 | 槽位 12/13/14/15/16 → `SKF_CreateFile/WriteFile/ReadFile/GetFileInfo/DeleteFile`（参数一一对应） | 建/查/删已通 ✅；读写需用户角色（设备规则） |
| 系统检测「CSP 未安装」 | Admin 按 `APPInfo.ini` 的 `CSPName` 查 `HKLM\SOFTWARE\Microsoft\Cryptography\Defaults\Provider\<name>`；本机实际注册的是 `Longmai GM3000 for HZ CSP V1.1` | 对齐 ini 的 `CSPLib`/`CSPName` 到本机已安装的 HZ 版 CSP（可改回，见 12.4） | 待用户复验 |

> **扇区类槽位（9/10 `M_Read/WriteSectors`、19 `M_ConstructMSCMapFiles`）仍返回「不支持」**：
> SKF（GM/T 0016）没有扇区级接口，无法如实映射。若「下载 ISO」走的是扇区路径，
> 需要先确认设备协议（不能做假成功）。日志会打印调用它的槽位与参数。

### 12.10 下载 ISO：槽位 9/10（扇区读写）用 `SKF_Transmit` 打通

`Admin_btn_DownISO` 走的是**扇区**路径（日志实证）：

```
C_Login(userType=1) → M_slot[ 9] a1=1 a2=0 a3=27 a4=<buf> → 0x00000054   ← 未实现时
```

槽位语义与帧格式（2.2.19 反汇编链条）：
`M_WriteSectors` 0x13B0 → 0x55F0 → 0x344B0 → 帧构造 0x5D1B0；
`M_ReadSectors` 0x1400 → 0x34530 → 0x5D280。其中：

| 事实 | 出处 |
|---|---|
| **每扇区 2048 字节** | `shl eax,0xb`（0x344B0） |
| **命令码：写 0x2A / 读 0x28** | `push 0x2a`（0x344BA 附近）/ `push 0x28`（0x3457A） |
| **16 字节头**：`[0]=cmd`、`[1..4]=起始扇区(大端)`、`[5..6]=扇区数(大端)`、`[7..15]=0` | 0x5D1B0 / 0x5D280 |
| 发送形态 `(上下文, 头, 0x10, 负载, 负载长度)` | 同上（`push 0x10` + 4 个参数） |

SKF 标准（GM/T 0016）没有扇区接口，但 SKF 模块导出了 **`SKF_Transmit(hDev, pbCommand, ulCommandLen, pbData, pulDataLen)`（5 参）**，
参数形状与厂商内部发送函数**完全一致** → 垫片用同一帧透传（`shim.c` 的 `case 9/10`）。

幂等自测（`GM3000Probe sectors`：读 0 号扇区 → 原样写回 → 再读比对）**通过**：

```
ext[10] M_ReadSectors(start=0, count=1) → 0x00000000  head=6F00…（读到真实扇区数据）
ext[9]  M_WriteSectors(原样写回)        → 0x00000000  ✅
ext[10] 回读比对                        → 0x00000000  ✅ 一致
```

后续若还需 `M_ConstructMSCMapFiles`（槽位 19，**2 参**：导出层 0x16B0 → 0x0C720），按同一思路处理。

> **用户实测：下载 ISO 成功**（此前固定报 `ErrorCode=0x00000054`）。

> 另：**厂商原版 `gm3000_pkcs11.dll` 认不到本设备**（`C_GetSlotList(present=1)` 返回 0，
> 4 个槽位全 invalid；它只依赖 KERNEL32/ADVAPI32/USER32/SETUPAPI/HID），
> 所以「临时换回原版 DLL 去下 ISO」这条路不通——垫片必须自己实现。

### 12.11 证书类型显示：厂商私有属性 `0x80000066` / `0x80000067`

Admin 建证书/密钥对象时会带上两个**厂商私有属性**（模板转储实证）：

| 属性 | 长度 | 含义 |
|---|---|---|
| `0x80000066` | 39 B | 容器名（与 `CKA_ID` 同值，含结尾 NUL） |
| `0x80000067` | 4 B | **1 = 签名证书/签名密钥，0 = 密钥交互（加密）** |

垫片原先**两者都不读、不回报**（`C_GetAttributeValue` 回 `CKR_UNAVAILABLE`），
Admin 因此无法判断证书角色 → 界面把同一张证书显示成「签名证书 + 密钥交互证书」两条。
现在：`C_CreateObject` 按 `0x80000067` 决定 `SKF_ImportCertificate(bSign)` 的角色顺序；
`C_GetAttributeValue` 如实回报这两个属性（证书对象在 `LoadObjects` 里按**导出成功的那个角色**记录）。
实测（`GM3000Probe ckobj`）：

```
厂商私有 0x80000066(容器名)=10B 'ZZOBJTEST'   0x80000067(签名/密钥交互)=4B 值=1
```

**设备侧证书清点**（`GM3000Probe skfcerts`，裸 SKF）也如实记录一下现状：

```
容器[2] = [Pikachu Common Code Sign SHA256 V02, {04CA735B-3599-4B99-9E54-8C096A48FD79}]
  'Pikachu…'   签名证书 rc=0x0 len=4842 sha256[0..8]=C61D9225B3B853F6
               密钥交互   rc=0x0A00001C（不存在）
  '{04CA…}'    签名证书 rc=0x0 len=4842 sha256[0..8]=C61D9225B3B853F6   ← 与上面**同一张**证书
               密钥交互   rc=0x0A00001C（不存在）
```

即设备上只有**一张**签名证书，被写进了**两个容器**（一次是早前用容器名=CN 导入的，一次是本次 Admin
按密钥 GUID 新建容器导入的）；并没有任何密钥交互证书。
