...
def disasm(rva, n=120):
    off = r2o(rva)
    return list(md.disasm(data[off:off + n * 16], rva))

# Show full prologue of HD_DeleteContainer to understand its 2 params
for name, rva in [('HD_DeleteContainer', 0x6A10), ('HD_DeleteCert', 0x7210)]:
    print('=' * 80)
    print(f'{name} (RVA 0x{rva:X})')
    print('=' * 80)
    for insn in disasm(rva, 70):
        extra = ''
